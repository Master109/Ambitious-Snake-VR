...
namespace AmbitiousSnake
{
	public class VR_UIPointer : MonoBehaviour
	{
		...
		private static VR_UIPointer _instance;
		public static VR_UIPointer instance 
		{ 
			...
			get
			{
				if (_instance == null)
				{
					...
					_instance = (VR_UIPointer)GameObject.Instantiate((VR_UIPointer)Resources.Load("Prefabs/GazePointerRing", typeof(VR_UIPointer)));
				}
				return _instance;
			}
		...
		void Update ()
		{
			...
			transform.position = VRCameraRig.CurrentHand.position + VRCameraRig.CurrentHand.forward * depth;
		...
		public void SetPosition(Vector3 pos, Vector3 normal)
		{
			...
			depth = (VRCameraRig.CurrentHand.position - pos).magnitude;
		...
		{
			SetPosition(pos, VRCameraRig.CurrentHand.forward);
		}
		...
		void LateUpdate()
		{
			...
			if (positionSetsThisFrame == 0)
			{
				...
				Quaternion newRot = transform.rotation;
				newRot.SetLookRotation(VRCameraRig.CurrentHand.forward, (lastPosition - transform.position).normalized);
				transform.rotation = newRot;
			}
